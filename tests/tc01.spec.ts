...
test("TC01 - Verify users can buy an item successfully", async ({ page }) => {
  ...
  await checkoutPage.verifyOrderItemDetail(productInfo);

  // // Step 15: Fill the billing details with default payment method
  await checkoutPage.fillBillingDetail(userInfo);
  ...
  await orderPage.verifyOrderDetail(productInfo);
  await orderPage.verifyBillingDetail(userInfo);
});
