...
test("TC09 - Verify users can update quantity of product in cart", async ({ page }) => {
  ...
  let productAmount = Number(await productInfoPage.storeProductAmount());
  ...
  await cartPage.verifyProductAmount(productName, productAmount);
  await cartPage.verifyTotalPrice(productName, productPrice, productAmount);
